...
class Trainer:
    ...
    def train(self):
        ...
        # # 🔹 모델 파라미터 수 / 크기 한 번만 출력 (rank 0에서만)
        # if self.rank == 0:
        #     model = self.model
        #     # DDP 래핑되어 있으면 안쪽 모듈 꺼내기
        ...
        #     model_size_mb = total_params * dtype_bytes / (1024 ** 2)

        #     print("=" * 60)
        #     print(f"[Model] total params      : {total_params:,}")
        #     print(f"[Model] trainable params  : {trainable_params:,}")
        #     print(f"[Model] dtype             : {dtype}")
        #     print(f"[Model] approx size       : {model_size_mb:.2f} MB")
        #     print("=" * 60)
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                for j in range(self.args.gradient_accumulation_steps):
                    ...
                    if self.rank == 0:
                        ...
                        msg = (f"[Step {self.progress['step']}/{self.total_step}] "
                            f"loss={average_loss:.4f}, top10acc={average_top10acc:.4f}, "
                            f"ntokens={sum_ntoken}")
                        for cb, acc_cb in enumerate(average_top10acc_cbi, start=1):
                            msg += f", top10acc_cb{cb}={acc_cb:.4f}"
                        print(msg)
    ...
    def validate_and_save(self):
        ...
        if self.rank == 0:
            ...
            save_path = os.path.join(self.args.exp_dir, f"bundle_step_{self.progress['step']}.pth")
            torch.save(
                {
                    "model": self.model.module.state_dict(),
                    "optimizer":  self.optimizer.state_dict(),
    ...
    def _setup_dataloader(self):
        ...
        train_dataset, val_dataset = ears.dataset(self.args, 'train'), ears.dataset(self.args, 'validation')
    ...
    def _setup_models(self):
        ...
        if self.args.load_model_from is not None and self.progress['step'] <= 1:
            # 1) 사전 학습된 체크포인트 로드
            ckpt = torch.load(self.args.load_model_from, map_location="cpu")['model']

            # 2) 텍스트 임베딩만 별도 처리
            old_w = ckpt['text_embedding.word_embeddings.weight']  # [101, 2048]
            new_w = model.text_embedding.word_embeddings.weight.data  # [111, 2048], 랜덤 초기화된 상태

            # 앞 101개 행만 복사
            new_w[:old_w.size(0), :] = old_w
            # 그리고 ckpt dict에 덮어쓰기
            ckpt['text_embedding.word_embeddings.weight'] = new_w
