...
class VoiceCraft(
    ...
    def prepare_mask_intervals_with_center(self, y_mask_center, y_lens):

        def _to_int(x): 
            return int(x.item()) if hasattr(x, "item") else int(x)

        B = len(y_lens)
        Lmin = int(self.args.mask_len_min)

        def clamp_Lmax(y_len):
            return int(max(Lmin, min(self.args.mask_len_max, max(2, y_len-2))))

        def sample_total_len(y_len):
            Lmax = clamp_Lmax(y_len)
            return random.randint(Lmin, Lmax)

        def symmetric_window(center, total_len, y_len):
            half = total_len // 2
            s = center - half
            e = s + total_len
            if s < 1:
            ...
            if e <= s: 
                e = min(y_len - 1, s + 1)
            s = max(1, min(s, y_len - 2))
            e = max(s + 1, min(e, y_len - 1))
            return s, e

        def sample_total_spans():
            msd = str(self.args.mask_sample_dist).lower()
            if msd == "uniform":
                return random.choice(range(1, int(self.args.max_n_spans) + 1))
            if "poisson" in msd:
                lam = float(msd[len("poisson"):])
                ps = torch.poisson(torch.tensor([lam]))
                return int(ps.clamp(1, int(self.args.max_n_spans)).item())
            return 1

        mask_intervals, non_mask_intervals = [], []

        for b in range(B):
            y_len = max(_to_int(y_lens[b]), 2)
            center = _to_int(y_mask_center[b]) if isinstance(y_mask_center, torch.Tensor) else int(y_mask_center[b])

            spans = []

            if center > 0:
                center = max(1, min(center, y_len - 1))
                L = sample_total_len(y_len)
                s, e = symmetric_window(center, L, y_len)
                spans.append((s, e))

            n_total = sample_total_spans()
            n_extra = max(0, n_total - (1 if center > 0 else 0))

            for _ in range(n_extra):
                c = random.randint(1, max(1, y_len - 1))
                L = sample_total_len(y_len)
                s, e = symmetric_window(c, L, y_len)
                spans.append((s, e))

            if not spans:
                c = max(1, (y_len - 1) // 2)
                s, e = symmetric_window(c, Lmin, y_len)
                spans = [(s, e)]

            spans.sort(key=lambda x: x[0])
            cleaned = []
            mgap = int(self.args.min_gap)
            for s, e in spans:
                if not cleaned:
                    cleaned.append([s, e]); continue
                ps, pe = cleaned[-1]
                if s <= pe or (s - pe) < mgap:
                    cleaned[-1][1] = max(pe, e)
                else:
                    cleaned.append([s, e])

            mask_spans = [(max(1, s), min(y_len - 1, e)) for (s, e) in cleaned if e > s]

            non_spans, cursor = [], 0
            for s, e in mask_spans:
                if s > cursor:
                    non_spans.append((cursor, s))
                cursor = e
            if cursor < y_len:
                non_spans.append((cursor, y_len))

            mask_intervals.append(mask_spans)
            non_mask_intervals.append(non_spans)
    ...
    def prepare_input_target(self, y, y_lens, y_mask_center=None):
        # rearrange y
        ...
        if self.args.emotion_driven_nv_matching:
            mask_intervals, non_mask_intervals = self.prepare_mask_intervals_with_center(y_mask_center, y_lens)
        else:
    ...
    def forward(self, batch):
        ...
        Args:
          ...
          y_lens:
            ...
            before padding.
        """
        x, x_lens, y, y_lens, y_mask_center = batch["x"], batch["x_lens"], batch["y"], batch["y_lens"], batch["y_mask_center"]
        ...
        y_input, new_y_lens, targets, y_padding_mask, y_attention_mask, mask_position, patterns = self.prepare_input_target(y, y_lens, y_mask_center)
